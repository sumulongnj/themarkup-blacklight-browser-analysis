...
export const collectFirefox = async (inUrl: string, args: CollectorOptions) => {
    ...
    logger.log('info', `browsing now to ${chosenLink}`, { type: 'Browser' });
    if (didBrowserDisconnect) {
        return {
            status: 'failed',
            page_response: 'Firefox crashed'
    ...
    console.log(`Browsing now to ${chosenLink}`);
    await navigateWithTimeout(page, chosenLink, args.defaultTimeout, args.defaultWaitUntil as 'load' | 'domcontentloaded' | 'networkidle');
