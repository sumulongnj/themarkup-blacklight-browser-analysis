                    ...
                    if (!page.isClosed()) {
                        ...
                        if (autoCompleteValue) {
                            if (autoCompleteValue.includes('cc-name')) {
                                ...
                                autoCompleteKeys = Object.keys(DEFAULT_INPUT_VALUES).filter(k => autoCompleteValue.includes(k));
                            }
                        }
                        ...
                        if (pValue === 'submit' || pValue === 'hidden') {
                            ...
                            await page.keyboard.type(DEFAULT_INPUT_VALUES[autoCompleteKeys[0]]);
                        } else if (Object.keys(DEFAULT_INPUT_VALUES).includes(pValue)) {
                            await el.focus();
                            await page.keyboard.press('Tab', { delay: 100 });
                            await el.press('Backspace');
                            await page.keyboard.type(DEFAULT_INPUT_VALUES[pValue]);
                        }
                        isInteracting = false;
                    } else {
                        break;
